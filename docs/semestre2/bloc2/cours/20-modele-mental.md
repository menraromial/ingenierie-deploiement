# Chapitre 20 : Le modèle mental de Kubernetes, la réconciliation
...
```mermaid
flowchart LR
    O["<b>Observer</b><br/>lire l'état RÉEL<br/>(quels Pods existent ?)"] --> C{"<b>Comparer</b><br/>réel = désiré ?"}
    C -->|"oui"| W["ne rien faire<br/>(attendre, ré-observer)"]
    C -->|"non"| A["<b>Agir</b><br/>créer / supprimer<br/>ce qu'il faut"]
    A --> O
    W --> O
```
...
```mermaid
flowchart TB
    subgraph CP["Plan de contrôle (control plane)"]
        API["<b>API Server</b><br/>la seule porte d'entrée ;<br/>valide et stocke les objets"]
        ETCD[("<b>etcd</b><br/>base clé-valeur ;<br/>LA source de vérité")]
        SCHED["<b>Scheduler</b><br/>choisit le nœud<br/>de chaque Pod"]
        CM["<b>Controller Manager</b><br/>fait tourner les<br/>boucles de réconciliation"]
        API <--> ETCD
        SCHED --> API
        CM --> API
    end
    subgraph N1["Nœud de travail 1"]
        K1["<b>kubelet</b><br/>agent : lance et<br/>surveille les Pods"]
        PROXY1["<b>kube-proxy</b><br/>règles réseau<br/>des Services"]
        RT1["Runtime (containerd/CRI-O)"]
        K1 --> RT1
    end
    subgraph N2["Nœud de travail 2"]
        K2["kubelet"]
        RT2["Runtime"]
        K2 --> RT2
    end
    API <-->|"le kubelet lit ce qu'il doit faire,<br/>rapporte l'état réel"| K1
    API <--> K2
```
...
sequenceDiagram
    ...
    U->>A: 1. apply Deployment (replicas:3)
    A->>A: valide (RBAC, schéma)
    A->>E: 2. écrit l'objet Deployment (état DÉSIRÉ)
    C->>A: 3. Deployment ctrl voit le Deployment → crée un ReplicaSet
    C->>A: 4. ReplicaSet ctrl voit "3 voulus, 0 réels" → crée 3 Pods (sans nœud)
    S->>A: 5. Scheduler voit 3 Pods non placés → leur assigne des nœuds
    K->>A: 6. kubelet voit "un Pod m'est assigné" → le lance via le runtime
    K->>A: 7. kubelet rapporte : Pod Running
    A->>E: état réel mis à jour
```

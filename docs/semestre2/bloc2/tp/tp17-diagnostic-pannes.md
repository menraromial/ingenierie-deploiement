# TP 17 : Casser pour comprendre, le diagnostic de pannes
...
```mermaid
flowchart LR
    G["<b>kubectl get pods</b><br/>QUEL est l'état ?<br/>(Pending, CrashLoop,<br/>ImagePull, OOMKilled...)"] --> D["<b>kubectl describe pod</b><br/>POURQUOI ?<br/>(les <b>Events</b> en bas)"]
    D --> L["<b>kubectl logs</b><br/>que dit l'APPLICATION ?<br/>(--previous si redémarré)"]
    L --> E["<b>kubectl get events</b><br/>que s'est-il passé<br/>dans l'ordre ?"]
```
...
!!! example "Exemple travaillé : une image inexistante"
...
??? note "Banque de pannes du bloc 2 (réservé enseignant, ne pas lire si vous jouez le jeu)"

# Chapitre 15 : Sous le capot, namespaces, cgroups, systèmes de fichiers en couches
...
!!! abstract "Objectifs du chapitre"
    ...
    - expliquer un système de fichiers en couches (OverlaySF) : lowerdir, upperdir, copy-on-write ;
...
```mermaid
flowchart LR
    subgraph C["Dans le conteneur"]
        R["root (UID 0)<br/>peut installer des paquets,<br/>écrire dans /etc..."]
    end
    subgraph H["Sur l'hôte"]
        U["UID 100000<br/>(un utilisateur SANS privilège,<br/>alloué dans /etc/subuid)"]
    end
    R -->|"user namespace : mapping"| U
```
...
!!! note "Lire un code de sortie : la convention `128 + signal`"
    ...
    |---|---|---|---|
    | SIGINT | 2 | **130** | vous faites ++ctrl+c++ |
...
Une image de conteneur doit être **partageable** (des dizaines de conteneurs partant de la même image `python:3.12` ne doivent pas dupliquer des centaines de Mo) et **immuable** (l'image ne doit jamais être modifiée par un conteneur en cours d'exécution). La solution est un **système de fichiers en couches** (*union filesystem*), sous Linux principalement **OverlaySF**.

### 4.2 Le mécanisme : lowerdir, upperdir, copy-on-write

OverlaySF **superpose** des répertoires et présente une vue fusionnée :

```mermaid
flowchart TB
    M["Vue fusionnée présentée au conteneur (sa racine /)"]
    U["<b>upperdir</b> : couche INSCRIPTIBLE<br/>propre à CE conteneur (vide au départ)"]
    L3["lowerdir 3 : votre code applicatif"]
    L2["lowerdir 2 : dépendances Python"]
    L1["lowerdir 1 : image de base (ex. debian-slim)"]
    U --> M
    L3 --> M
    L2 --> M
    L1 --> M
...
sequenceDiagram
    ...
    U->>P: podman run debian sleep 100
    P->>P: 1. Récupère l'image (couches) depuis le stockage local/registre
    P->>K: 2. Monte un OverlaySF (lowerdir=image, upperdir=neuf)
    P->>K: 3. Crée les namespaces (pid, net, mnt, uts, ipc, user)
    P->>K: 4. Crée un cgroup et y applique les limites
    P->>K: 5. Lance "sleep 100" comme PID 1, chroot sur la vue fusionnée
    K-->>P: Le processus tourne, isolé et borné
```
...
4. **OverlaySF** : lowerdir (couches image, lecture seule, partagées) + upperdir (inscriptible, par conteneur) + **copy-on-write**. Conséquences : images en couches partagées, conteneur **éphémère** (d'où les volumes), l'**ordre des couches** gouverne le cache de build.
...
- Documentation OverlaySF du noyau : `Documentation/filesystems/overlayfs.rst`.
